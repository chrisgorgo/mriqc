def create_report(subject_id, tsnr_file, realignment_parameters_file, mean_epi_file, mask_file, reg_file, fssubjects_dir, similarity_distribution, mean_FD_distribution, tsnr_distributions, output_file,t2_file,mean_FID_distribution):
    ...
    from mriqc.motion import plot_frame_displacement, plot_frame_intensity_displacement
    ...
    fig = plot_frame_displacement(realignment_parameters_file, mean_FD_distribution, threshold=2.0, figsize=(8.3, 8.3))
    report.savefig(fig, dpi=300)
    fig.clf()
    plt.close()

    fig = plot_frame_intensity_displacement(t2_file, mean_FID_distribution, threshold=3.0, figsize=(8.3, 8.3))
    report.savefig(fig, dpi=300)
    fig.clf()
    plt.close()
